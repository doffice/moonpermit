...
| 只记录 Allow/Deny 文本 | 收据保留请求、时间、字节成本、授权和剩余预算，可确定性重放 |
...
当前基线为 55 项测试全部通过，核心覆盖率 95.0%。相同门禁已在干净克隆和
...
- `runtime.mbt`：最窄授权优先、原子预算消费和收据生成。
...
v0.1 的“证明”指可重放的结构化决策证据，不声称密码学真实性。收据篡改会在
与可信输入比较或重放时暴露，但持久化、签名和可信时间属于宿主或后续版本。
...
收据和离线审计。通用配置加载、操作系统执行、分布式状态、持久化以及密码学签名
不在本期承诺内，避免把原型包装成未经验证的生产安全产品。
